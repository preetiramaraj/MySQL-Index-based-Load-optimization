                ...
                bool read_file_from_client)
{
  ...
  */
  ...
  // Drop secondary indexes before load
  bool alter_required;
  bool result_alter;
  LEX *lex = thd->lex;
  ...
  // Initializing values to read key data
  TABLE *alter_table = table_list->table;
  HA_CREATE_INFO create_info(lex->create_info);
  Alter_info add_alter_info;
  Alter_info drop_alter_info;
  Alter_drop *ad;
  Key* add_key;
  char* add_key_name;
  char *field_name;
  if (alter_table->s->keys > 0)
  {
    alter_required = true;
  }
  if(alter_required)
  {
   ...
   KEY *key_info =alter_table->s->key_info;
   for (uint i=0 ; i < alter_table->s->keys ; i++, key_info++)
   {
      // Keys to be dropped
      ad = new Alter_drop(Alter_drop::KEY, key_info->name);
      drop_alter_info.drop_list.push_back(ad);

      // Alter information for add keys
      ...
      {
         field_name = new char[strlen(key_part->field->field_name)];
         strcpy(field_name, key_part->field ? key_part->field->field_name : "?unknown field?");
         lex->col_list.push_back(new Key_part_spec(field_name,
                                                 strlen(field_name), 0));
      }
      // Creating key to be added to the alter info key list
      add_key_name = new char[strlen(key_info->name)];
      strcpy(add_key_name, key_info->name);
      add_key = new Key(KEYTYPE_MULTIPLE,
                       add_key_name,
                       strlen(add_key_name),
                       &key_create_info,
                       false,
                       lex->col_list);
     add_alter_info.key_list.push_back(add_key);
     lex->col_list.empty();  
   }
    
   // Initializing flags as required
   add_alter_info.flags|= Alter_info::ALTER_ADD_INDEX;
   drop_alter_info.flags|= Alter_info::ALTER_DROP_INDEX;
 
   // Following steps required to close the table after use in preparation
   // for mysql_alter_table
   ...
   Alter_info alter_info(drop_alter_info, thd->mem_root);

   result_alter = mysql_alter_table(thd, 
                                    db, 
				    lex->name.str,
                       		    &create_info,
                       		    table_list,
 ...
 // Restoring back to original state to resume original execution, since the table was opened or the alter statement was executed
 thd->lock = NULL;
 table_list->table = NULL;
 thd->lex->lock_tables_state = Query_tables_list::LTS_NOT_LOCKED;
 table_list->mdl_request.ticket = NULL; 
  ...
  */
  ...
  if(alter_required)
  {
    // Adding back the indexes after setting state for calling Alter_table again
    // Restoring back to original state of thread
    mysql_unlock_tables(thd, thd->lock);
    thd->lock = NULL;
    thd->lex->lock_tables_state = Query_tables_list::LTS_NOT_LOCKED;
    table_list->mdl_request.ticket = NULL; 
    // Assigning values based on indexes that existed on the table
    HA_CREATE_INFO add_create_info(lex->create_info);
    Alter_info add_key_alter_info(add_alter_info, thd->mem_root);
    bool result_alter_add_indexes_back;	
    result_alter_add_indexes_back = mysql_alter_table(thd, 
                                                      db, 
				                      lex->name.str,
                                                      &add_create_info, 
                         		                    table_list,
                         		                    &add_key_alter_info);
    if (result_alter_add_indexes_back)
    {
       DBUG_RETURN(true);
    }

    // Restoring states so that tables can be released as per the mechanisms of Load
    thd->lock = NULL;
    thd->lex->lock_tables_state = Query_tables_list::LTS_NOT_LOCKED;
    table_list->mdl_request.ticket = NULL; 
    thd->get_stmt_da()->reset_diagnostics_area();
    ...
    // Freeing memory of variables created
    free(add_key_name);
    free(field_name);
  }
