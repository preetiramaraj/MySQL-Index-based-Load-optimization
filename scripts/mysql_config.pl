...
my $flags;
$flags->{libs} = [@lib_opts, qw(-lpthread -lm -lrt -ldl)];
$flags->{embedded_libs} = [@lib_e_opts, qw(-lpthread -lm -lrt -lcrypt -ldl)];

$flags->{include} = ["-I$pkgincludedir"];
$flags->{cflags}  = [@{$flags->{include}},split(" ",'-fabi-version=2 -fno-omit-frame-pointer')];
$flags->{cxxflags}= [@{$flags->{include}},split(" ",'-fabi-version=2 -fno-omit-frame-pointer')];
